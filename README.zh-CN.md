...
Claw 生态（OpenClaw、QClaw、WorkBuddy、ZeroClaw、PicoClaw、KimiCLI、ClawCoder、QuectoClaw 等）近期爆火。很多用户只是尝鲜，却给了大量权限，导致一系列安全隐患。**KillClaws** 一键扫描系统中已安装的 Claw 产品，显示详细信息，让你选择要卸载哪些，然后彻底清理——配置文件、包管理器安装包、后台服务、二进制文件——全部搞定。
...
| **ClawCoder** | Jason Sang | pip 包（clawcoder）、启动器（clawcode/clawcoder）、配置文件（~/.clawcoder）、Windows 安装目录（%LOCALAPPDATA%\ClawCoder） |
| **QuectoClaw** | 社区 | 二进制文件 / Homebrew 公式（quectoclaw）、配置与工作区（~/.quectoclaw） |
